...
Java_com_kangwang_ffmpeddemo_FFmpegdiaPlayer_initPlay(JNIEnv *env, jobject thiz, jstring path,
    ...
    VideoPlay *videoPlay = new VideoPlay(env,char_path,surface);
    return reinterpret_cast<jlong>(videoPlay);
}extern "C"
JNIEXPORT void JNICALL
Java_com_kangwang_ffmpeddemo_FFmpegdiaPlayer_player(JNIEnv *env, jobject thiz, jstring absolute_path,
                                                    jobject surface) {
    //  渲染
    ANativeWindow  *nativeWindow = ANativeWindow_fromSurface(env,surface);
//  将数据写入 到 缓存区
